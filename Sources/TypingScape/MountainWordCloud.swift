...
/// packed, no separate outline drawn. A final pixel-mask `.mask()` over
/// everything sharpens that to the silhouette's actual boundary — small
        ...
        GeometryReader { geo in
            if let mask, !flow.isEmpty, let croppedMask = mask.croppedDensityContent() {
                ...
                Canvas { context, size in
                    ...
                    // A word typed often should visibly outgrow a one-off
                    // word, not render at the same size — the row height
                    ...
                    // Fills one contiguous span of a row — a row can have
                    // more than one (a sea's separate wave crests, a
                    // star's two feet), and each needs its own justified
                    // pass so no span is left blank.
                    func fillSpan(rowY: CGFloat, minX: CGFloat, maxX: CGFloat) {
                        let rowWidth = maxX - minX
                        // Near a shape's narrow extremities (a star's
                        // point, a mountain's peak) not even the smallest
                        // frequency size fits — shrink the cap itself
                        // instead of skipping the row outright, so text
                        ...
                        // Pass 1: greedily collect whichever words fit
                        // this span, measuring at a nominal minimum gap.
                        // Each word's size comes from how often it was
                        // typed today, capped by whatever actually fits
                        // this span.
                        ...
                        while wordIndex < flow.count {
                            ...
                            accumulated = needed
                            // A word sitting exactly on the row's ruled
                            // line, every row perfectly evenly spaced,
                            // reads as a spreadsheet rather than
                            // something hand-set — a small per-word
                        ...
                        // Pass 2: justify — spread the leftover space
                        // evenly across the gaps so the span's
                        // first/last word touch minX/maxX.
                    ...
                    for rowFromBottom in 0..<rowCount {
                        // Rows this evenly spaced, row after row, is the
                        // other half of the "ruled grid" look — nudging
                        // each one keeps them from crossing (bounded well
                        // under half the row height) while breaking up
                        // the rhythm.
                        ...
                        // Map this row's position within the cropped
                        // content back to the mask's own full-image
                        // fraction space before sampling.
                ...
                // Sharpens the row/span fill above to the mask's real
                // pixel boundary — cuts away anything that spilled past
                // an edge, and (unlike the row sampling) always carves
                // out small interior holes like a carved-out eye/mouth
                // correctly regardless of whether a sampled row happened
                // to land on them.
                .mask(
                    Image(decorative: croppedMask, scale: 1)
                        .resizable()
